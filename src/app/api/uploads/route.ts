...
import { readImageDimensions, bytesMatchMime } from "@/lib/image-dimensions";
...
export async function POST(req: NextRequest) {
  ...
  // `mime` came from File.type, i.e. the client. Verify the bytes actually
  // are that format before storing them under that content-type — otherwise
  // the allowlist above only filters what a well-behaved client declares,
  // and the public bucket accepts arbitrary blobs from any signed-in user.
  if (!bytesMatchMime(bytes, mime)) {
    return NextResponse.json({ error: "File contents don't match its image type." }, { status: 415 });
  }
