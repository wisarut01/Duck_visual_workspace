...
import { readImageDimensions, bytesMatchMime } from "./image-dimensions";
...
describe("bytesMatchMime", () => {
  it("accepts bytes whose signature matches the claimed type", () => {
    expect(bytesMatchMime(pngHeader(4, 2), "image/png")).toBe(true);
    expect(bytesMatchMime(gifHeader(6, 3), "image/gif")).toBe(true);
  });

  it("rejects a spoofed content type (the bytes are a GIF, the claim is PNG)", () => {
    expect(bytesMatchMime(gifHeader(6, 3), "image/png")).toBe(false);
  });

  it("rejects arbitrary non-image bytes claiming to be an image", () => {
    const junk = new TextEncoder().encode("<svg onload=alert(1)></svg>");
    for (const mime of ["image/png", "image/gif", "image/jpeg", "image/webp"]) {
      expect(bytesMatchMime(junk, mime), mime).toBe(false);
    }
  });

  it("accepts a RIFF/WEBP container header", () => {
    const buf = new Uint8Array(16);
    buf.set(new TextEncoder().encode("RIFF"), 0);
    buf.set(new TextEncoder().encode("WEBP"), 8);
    expect(bytesMatchMime(buf, "image/webp")).toBe(true);
  });
});
